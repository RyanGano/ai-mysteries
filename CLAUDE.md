...
Tags appear on catalog cards and drive filtering. Two rules to hold in mind here; everything
else lives in the reference file.

1. **Tag setting, era, and subject too — not just plot mechanics** (crime type, audience).
   Whenever something is load-bearing to the story (a boat, a decade, a sport, a profession),
   it earns a tag. A thin tag list under-serves filtering as much as a bloated one.
2. **The live catalog is the source of truth for which tags exist** — not any file in this repo.
   Tags are book data (`BookMeta.tags`) in Cosmos. Fetch `GET /api/books` and union the `tags`
   arrays to see the real list; reuse an existing tag whenever the meaning is close, since two
   near-synonyms just fragment the same filter.

**Before tagging a book, read [`tag_glossary.md`](tag_glossary.md)** — the full four-step
procedure and the semantic glossary of what each tag means. It is kept out of this file on
purpose (it is consulted a few times per book, not every turn), so go read it rather than
tagging from memory.
