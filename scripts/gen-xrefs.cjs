...
// sections drive two committed artifacts under ai-mysteries-web/src/content/endings/:
//
//   * Clue Library                       -> clues.ts        (clueId -> { chapter, quote })
//   * Marker placement (annotated …)     -> xref-markers.ts (endingCode -> { slug, markers })
//
// Run with:  node scripts/gen-xrefs.cjs
//
// The .md doc is never committed (see source_materials / docs policy in CLAUDE.md), so this
// script only runs locally where the doc is present. The generated .ts files ARE committed.
//
...
function emit() {
  ...
  const cluesFile = `// AUTO-GENERATED by scripts/gen-xrefs.cjs — do not edit by hand.
...
`;
  ...
  const markersFile = `// AUTO-GENERATED by scripts/gen-xrefs.cjs — do not edit by hand.
...
`;
  ...
  fs.writeFileSync(path.join(ENDINGS_DIR, "clues.ts"), cluesFile);
  fs.writeFileSync(path.join(ENDINGS_DIR, "xref-markers.ts"), markersFile);

  const markerCount = Object.values(byCode).reduce((n, e) => n + e.markers.length, 0);
  console.log(
    `Wrote clues.ts (${Object.keys(clues).length} clues) and xref-markers.ts ` +
